Main_new<-function(input, seed, traits, K, obj_best, Iter){
  
  DB<-read.csv(input, fileEncoding="latin1")
  DB_ori<-DB
  
  # Keep all training/testing initial/simulated data in lists
  val_result<-list("Lab"=c(), "Simu"=c())
  tr_result<-list("Lab"=c(), "Simu"=c())
  G_each_K<-c()
  E_each_K<-c()
  # Keep indicies of data as well
  va_idx_all<-c()
  tr_idx_all<-c()
  ...
  # Create gene matrix (X) and environmental condition matrix (Y)
  ...
  # For X and Y, if there is a condition that has not more than 3 cases with non-zero values, delete it, together with cases carrying non-zero values
  ...
  # Keep the indices of data used in each round of training or testing
  for (i in 1:K){
  ...
  for (i in 1:num_papers){
    ...
    # For unknown reason which(DB[, "Author"]==papers[i]) does not work here, so a clumsy strategy is used here to collect the row numbers of data points from the same paper
    ...
    # For data from the same paper, keep 1/K th for testing and the remaining for training
    for (j in 1:K){
      if (num_selected==0){
  ...
  # Randomize the order of data points in the training/testing set
  for (i in 1:K){
  ...
  for (i in 1:K){
    ...
    # Take out indices of data points for use 
    ...
    # Main body of training, return all useful information about the training
    result_train[[i]]<-Train_SA(num_exp, G, E, X_tr, Y_tr, GR_tr, obj_best, T_range, Iter)
    ...
    # Keep the lab and simulated growth rates in separate vectors
