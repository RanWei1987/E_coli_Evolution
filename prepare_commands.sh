...
exp_unique_num=$(awk 'NR==1 {next} {print $1}' ../Merged_Data_4.csv | cut -d "," -f 3 | uniq | wc -l)
traits=8
seed=5
K=10
obj_best=0.99
...
for ((trait=1; trait<=traits; trait=trait*2)); do
	for ((i=1; i<=$exp_unique_num; i=i+10)); do 
		...
		for ((j=i; j<=last_j; j++)); do
			exp_index=$(awk 'BEGIN {FS=","} NR==1 {next} $3 == "'$j'" {print NR-1}' ../Merged_Data_4.csv)
			arr=($exp_index)
			start=${arr[0]}
			end=${arr[-1]}
			current_exp=$(($(($j-$i))*3))
		...
		echo "cd ./seed_${seed}traits_$trait; Rscript Run_shell_C.R ../../Merged_Data_4.csv $seed $trait $obj_best $Iter ${exp_unique_index[@]}" >> list_of_commands.txt
	done
done
